...
use susy_jsonrpc_core::{Error, Params, Value, Metadata, Result};
use susy_jsonrpc_core::futures::{self, Future, IntoFuture};
use susy_jsonrpc_core::futures::future::{self, Either};
use susy_jsonrpc_pubsub::{PubSubMetadata, Subscriber};
use pubsub;
use serde::Serialize;
...
macro_rules! metadata {
	() => {
		/// Requests metadata
		type Metadata: $crate::susy_jsonrpc_core::Metadata;
	};
	(
		$( $sub_name: ident )+
	) => {
		/// Requests metadata
		type Metadata: $crate::susy_jsonrpc_pubsub::PubSubMetadata;
	};
}
...
macro_rules! build_rpc_trait {
	...
	(
		...
		pub trait $name: Sized + Send + Sync + 'static {
			...
			/// the parameters.
			fn to_delegate<M: $crate::susy_jsonrpc_core::Metadata>(self) -> $crate::IoDelegate<Self, M> {
	...
	( WRAP $del: expr =>
		...
		$del.add_subscription(
			...
			($unsubscribe, move |base, id| {
				use $crate::susy_jsonrpc_core::futures::{IntoFuture, Future};
