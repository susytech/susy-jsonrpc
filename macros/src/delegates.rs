...
use susy_jsonrpc_core::{Params, Value, Error};
use susy_jsonrpc_core::{BoxFuture, Metadata, RemoteProcedure, RpcMethod, RpcNotification};
use susy_jsonrpc_core::futures::IntoFuture;

use susy_jsonrpc_pubsub::{self, SubscriptionId, Subscriber, PubSubMetadata};
...
impl<T, M, F> susy_jsonrpc_pubsub::SubscribeRpcMethod<M> for DelegateSubscribe<T, F> where
	M: PubSubMetadata,
...
impl<M, T, F, I> susy_jsonrpc_pubsub::UnsubscribeRpcMethod<M> for DelegateUnsubscribe<T, F> where
	M: PubSubMetadata,
	...
	M: PubSubMetadata,
{
	...
	pub fn add_subscription<Sub, Unsub, I>(
		...
		I::Future: Send + 'static,
	{
		let (sub, unsub) = susy_jsonrpc_pubsub::new_subscription(
			name,
			DelegateSubscribe {
				delegate: self.delegate.clone(),
				closure: subscribe.1,
