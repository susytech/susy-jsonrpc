	...
	fn params_with_trailing(
		...
		quote! {
			let passed_args_num = match params {
				_susy_jsonrpc_core::Params::Array(ref v) => Ok(v.len()),
				_susy_jsonrpc_core::Params::None => Ok(0),
				_ => Err(_susy_jsonrpc_core::Error::invalid_params("`params` should be an array"))
			};

			let params = passed_args_num.and_then(|passed_args_num| {
				match passed_args_num {
					_ if passed_args_num < #required_args_num => Err(_susy_jsonrpc_core::Error::invalid_params(
						format!("`params` should have at least {} argument(s)", #required_args_num))),
					#(#switch_branches),*,
					_ => Err(_susy_jsonrpc_core::Error::invalid_params_with_details(
