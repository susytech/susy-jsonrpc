...
pub fn generate_client_module(methods: &[MethodRegistration], item_trait: &syn::ItemTrait) -> Result<TokenStream> {
	...
	let (markers_decl, markers_impl): (Vec<_>, Vec<_>) = markers
		...
		.unzip();
	let client_name = crate_name("susy-jsonrpc-client")?;
	Ok(quote! {
		/// The generated client module.
		pub mod gen_client {
			use #client_name as _susy_jsonrpc_client;
			use super::*;
			use _susy_jsonrpc_core::{
				Call, Error, ErrorCode, Id, MethodCall, Params, Request,
				Response, Version,
			};
			use _susy_jsonrpc_core::futures::{future, Future, Sink};
			use _susy_jsonrpc_core::futures::sync::oneshot;
			use _susy_jsonrpc_core::serde_json::{self, Value};
			use _susy_jsonrpc_client::{RpcChannel, RpcError, RpcFuture, RpcMessage};
