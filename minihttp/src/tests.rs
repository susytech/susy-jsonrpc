...
use self::susy_jsonrpc_core::{IoHandler, Params, Value, Error};
use self::susy_jsonrpc_core::futures::{self, Future};
...
fn serve_hosts(hosts: Vec<hosts::Host>) -> Server {
	...
	ServerBuilder::new(IoHandler::default())
		.cors(hosts::DomainsValidation::AllowOnly(vec![cors::AccessControlAllowOrigin::Value("http://superstring.ch".into())]))
...
fn serve() -> Server {
	...
	ServerBuilder::new(io)
		.cors(hosts::DomainsValidation::AllowOnly(vec![
			cors::AccessControlAllowOrigin::Value("http://superstring.ch".into()),
...
fn should_add_cors_allow_origins() {
	...
	let response = request(server,
		...
		{
			let mut headers = content_type_json();
			headers.set(header::Origin::new("http", "superstring.ch", None));
			headers
		},
		r#"{"jsonrpc":"2.0","id":1,"method":"x"}"#,
	);
	...
	assert_eq!(
		...
		Some(&reqwest::header::AccessControlAllowOrigin::Value("http://superstring.ch".into()))
	);
}
...
fn should_allow_if_host_is_valid() {
	...
	let response = request(server,
		...
		{
			let mut headers = content_type_json();
			headers.set_raw("Host", vec![b"superstring.ch".to_vec()]);
			headers
		},
		r#"{"jsonrpc":"2.0","id":1,"method":"x"}"#,
	);
