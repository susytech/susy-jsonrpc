...
extern crate bytes;
extern crate susy_jsonrpc_server_utils;
...
use susy_jsonrpc_server_utils::hosts;

pub use susy_jsonrpc_server_utils::cors;
pub use susy_jsonrpc_server_utils::hosts::{Host, DomainsValidation};
pub use req::Req;
...
pub struct ServerBuilder<M: jsonrpc::Metadata = (), S: jsonrpc::Middleware<M> = jsonrpc::middleware::Noop> {
	susy_jsonrpc_handler: Arc<MetaIoHandler<M, S>>,
...
impl<M: jsonrpc::Metadata, S: jsonrpc::Middleware<M>> ServerBuilder<M, S> {
	...
	{
		ServerBuilder {
			susy_jsonrpc_handler: Arc::new(handler.into()),
	...
	pub fn start_http(self, addr: &SocketAddr) -> io::Result<Server> {
		...
		let handler = self.susy_jsonrpc_handler;
