		...
		match response {
			...
			Err(request) => {
				Handler::Rpc(RpcHandler {
					susy_jsonrpc_handler: self.susy_jsonrpc_handler.clone(),
...
impl<M: Metadata, S: Middleware<M>> RpcHandler<M, S> {
	fn read_headers(
		...
		// Read metadata
		let metadata = self.susy_jsonrpc_handler.extractor.read_metadata(&request);
	...
	fn process_health(
		...
		Ok(RpcPollState::Ready(RpcHandlerState::WaitingForResponse(
			future::Either::B(self.susy_jsonrpc_handler.handler.handle_rpc_request(call, metadata)).map(|res| match res {
	...
	fn process_rest(
		...
		Ok(RpcPollState::Ready(RpcHandlerState::Waiting(
			future::Either::B(self.susy_jsonrpc_handler.handler.handle_rpc_request(call, metadata)).map(|res| {
			...
			match body.poll()? {
				...
				Async::Ready(None) => {
					...
					return Ok(RpcPollState::Ready(RpcHandlerState::Waiting(
						self.susy_jsonrpc_handler.handler.handle_request(content, metadata),
					)));
				}
				Async::NotReady => {
...
mod test {
	...
	use susy_jsonrpc_core::middleware::Noop;
