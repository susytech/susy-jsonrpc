...
use self::susy_jsonrpc_core::{Error, ErrorCode, IoHandler, Params, Value};
...
fn should_add_cors_allow_origins() {
	...
	assert!(
		response
			.headers
			.contains("access-control-allow-origin: http://superstring.ch"),
		"Headers missing in {}",
		response.headers
	);
}
