...
mod tests {
	...
	fn should_parse_host() {
		...
		assert_eq!(
			Host::parse("http://superstring.ch:8443"),
			Host::new("superstring.ch", Some(8443))
		);
		assert_eq!(
	...
	fn should_accept_if_on_the_list() {
		let valid = is_host_valid(Some("superstring.ch"), &Some(vec!["superstring.ch".into()]));
		assert_eq!(valid, true);
	}

	#[test]
	fn should_accept_if_on_the_list_with_port() {
		let valid = is_host_valid(Some("superstring.ch:443"), &Some(vec!["superstring.ch:443".into()]));
		assert_eq!(valid, true);
	}

	#[test]
	fn should_support_wildcards() {
		let valid = is_host_valid(Some("susy.web3.site:8180"), &Some(vec!["*.web3.site:*".into()]));
		assert_eq!(valid, true);
	}
}
