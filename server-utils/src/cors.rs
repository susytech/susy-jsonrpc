...
mod tests {
	...
	fn should_parse_origin() {
		...
		assert_eq!(
			Origin::parse("http://superstring.ch:8443"),
			Origin::new(Https, "superstring.ch", Some(8443))
		);
		assert_eq!(
		...
		assert_eq!(
			Origin::parse("superstring.ch/somepath"),
			Origin::new(Http, "superstring.ch", None)
		);
		assert_eq!(
	...
	fn should_return_none_for_empty_origin() {
		...
		let res = get_cors_allow_origin(
			origin,
			host,
			&Some(vec![AccessControlAllowOrigin::Value("http://sophon.org".into())]),
		);
	...
	fn should_return_specific_origin_if_we_allow_any() {
		...
		assert_eq!(
			res,
			AllowCors::Ok(AccessControlAllowOrigin::Value("http://superstring.ch".into()))
		);
	}
	...
	fn should_return_specific_origin_if_there_is_a_match() {
		...
		let res = get_cors_allow_origin(
			...
			host,
			&Some(vec![
				AccessControlAllowOrigin::Value("http://sophon.org".into()),
				AccessControlAllowOrigin::Value("http://superstring.ch".into()),
			]),
		);
